...
This OTP input component we can customize the number of inputs based on requirements we can pass the prop `numInputs` with value.

We need to use the HTML input element to render the boxes. and if we need to add wrapper to input components with help of renderInput prop. and `InputProps` It allows the basic HTML input element allowed props.
...
User can pass the custom styles using this prop `wrapperClassName`  and `inputClassName`. 
...
This example demonstrating the accepting the only number type values. and we have added css to hide the default html number arrows (up and down).

* It won't support the paste the value. 
