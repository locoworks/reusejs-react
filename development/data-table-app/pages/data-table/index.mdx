...
**Note:**
- To utilize the Simple `DataTable` with Pagination or you require `Search` and `Filter` functionalities in addition to `Pagination`  then you should utilize `HeadlessDataTable`. 
- The `HeadlessDataTable` component uses the `useDataTable` hook from the `@locoworks/reusejs-toolkit-react-hooks` library to manage `pagination` and other functionalities. The hook returns various properties that are used to render the table and handle `pagination`. Please refer to the hook section to get more visiblity
- When you use the `HeadlessDataTable` component, it automatically `paginates` the `tableData` based on the `itemsPerPage` prop. It also sets up navigation buttons to move between pages of data.
