...
To use the `ProgressBar` component, install the required dependencies:
...
The following exported components are available from the `@locoworks/reusejs-react-progress-bar` package:
...
Styling the `ProgressBar` component can be done by passing Tailwind CSS or custom CSS to the props:
`progressClasses`
`progressContainerClasses`

Styling the `CircularProgressBar` component can be done by passing Tailwind CSS or custom CSS to the props:
