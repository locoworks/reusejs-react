...
The CircularProgressBar accepts following 4 additional props apart from above:
- **radius** (optional as number): The radius of the circular progress bar (default: 95).
- **circleRadiusInPercentage** (optional as string): The radius of the circular progress bar in percentage (both radius and `circleRadiusInPercentage` need to be passed together ex: radius={55} , circleRadiusInPercentage="25%").
- **strokeColour** (optional): Takes CSS Classes for the strock colour and strock width of `CircularProgressBar`(example "bg-gray-500 strock-[10]").
- **progressColour** (optional): Takes CSS Classes for the Progress of the `CircularProgressBar`(example "bg-red-500 strock-[10]").
