...
import ForwardRefSelectDropdown from "../../components/select-dropdown/ForwardRefSelectDropdown/ForwardRefSelectDropdown.tsx"
...
```js
npm install @locoworks/reusejs-react-select-dropdown
```
or 
```js
yarn add @locoworks/reusejs-react-select-dropdown
```

## Importing

To Import `ReuseSelectDropdown`:
...
```js path=../../components/select-dropdown/ForwardRefSelectDropdown name=ForwardRefSelectDropdown
```
### useHeadlessSelectHook Component
...
Styling the `ReuseSelectDropDown` component can be done by passing Tailwind CSS or custom CSS to the props:
`optionsWrapper`
`selectWrapper`
`selectButtonStyles`
